...
# слайдбар для выбора фильма и отображения его данных
with st.sidebar:
    ...
    current_movie = next(
        m for m in st.session_state.movies if m['title'] == selected_name
    )
...
st.title("🎬 Система советника по фильмам")
    ...
    with chat_container:
        ...
        if user_input := st.chat_input("Спроси про жанр или год..."):
            ...
            if recommended_movie:
                ...
                validation_result = check_rules(recommended_movie)

                if validation_result:
                    validation_text = "🟢 Валидация: фильм соответствует правилам."
                else:
                    validation_text = "🔴 Валидация: обнаружено нарушение логических правил."

                final_answer = f"""
🎬 **Рекомендация:** {recommended_movie['title']}
...
"""
                ...
                poster_url = recommended_movie.get("poster", None)

            else:
                final_answer = f"{answer}\n\n⚠️ Постер и валидация не найдены."
                poster_url = None
