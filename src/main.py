...
# Импортируем новую функцию деталей
...
with st.sidebar:
    ...
    if st.session_state.movies:
        ...
        if st.button("Анализировать фильм"):
            ...
            with st.spinner('Загрузка данных из TMDB...'):
                # Получаем и постер, и описание
    ...
    if user_input := st.chat_input("Спроси про жанр или год..."):
        ...
        if recommended_movies:
            for idx, movie in enumerate(recommended_movies, 1):
                ...
                # Получаем детали через API
                movie_info = get_movie_details(movie['title'], movie.get('year'))
                
                poster_html = f'<br><img src="{movie_info["poster"]}" width="220" style="border-radius:10px;"><br>'
